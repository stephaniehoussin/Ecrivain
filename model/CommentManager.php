...
class CommentManager extends Manager
{
   ...
   {
        $comments = [];
        $req = $this->db()->query('SELECT id,postId, author, comment,is_signaled, DATE_FORMAT(comment_date, \'%d/%m/%Y à %Hh%i\')AS comment_date  FROM comments ORDER BY comment_date DESC ');
   ...
   {
        $req= $this->db()->prepare('UPDATE comments SET is_signaled= 1 WHERE id='.$id);
        $req->execute();
   }
